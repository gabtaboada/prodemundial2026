                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table">
									...
									<form action="{{ url('admin/participantes') }}" method="GET" class="navbar-form navbar-left pull-left" role="search">
											<div class="form-group">
											  	<input type="text" name="ParticipanteBusqueda" value="{{ request('ParticipanteBusqueda') }}" class="form-control" placeholder="Apellido / Nombre">
											</div>
											<div class="form-group">
												<select name="pago" class="form-control">
													@foreach($pago as $key => $value)
														<option value="{{ $key }}" {{ request('pago') == $key ? 'selected' : '' }}>{{ $value }}</option>
													@endforeach
												</select>
											</div>											  
					   					    <div class="form-group">
           							...
           							<p>
			                			<em style="font-size:12px;">Total de participantes que pagaron : {!!$TotalPagaron!!} </em>
			          			 	</p>	
                                    <thead>
                                        <tr>
                                            <th>Nombre</th>
										  ...
										  <tr class="warning">	
											...
											<td>
												@if($participante->Apuesta == "Tiene")
													<a href="{{ url('/admin/participantes/VerBoleta') }}/{{ $participante->id }}" style="text-decoration:none;">
														Ver boleta
														&nbsp;&nbsp;
														<img src="/images/edit.png" width="20" height="20" alt="" title="">
													</a> 
												@else
													<a href="{{ url('/admin/apuestas/cargarBoleta') }}/{{ $participante->id }}" style="text-decoration:none;">
														Cargar boleta
														&nbsp;&nbsp;
											...
											<td>
												<a href="{{ url('admin/participantes/'.$participante->id.'/edit') }}" class="btn btn-primary">Editar</a>
											</td>
											<td>
												<form action="{{ url('admin/participantes/'.$participante->id) }}" method="POST">
													{{ csrf_field() }}
