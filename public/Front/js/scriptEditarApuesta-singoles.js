switch ( window.location.host ){
	...
	case "www.prodeamigosqatar2022.com.ar":
			//var RutaGlobal = "https://www.gtecno.com.ar"+window.location.pathname;
			var RutaGlobal = "{{ asset('') }}prueba/boleta";
	break;	
	case "prodeamigosqatar2022.com.ar":
			//var RutaGlobal = "https://www.gtecno.com.ar"+window.location.pathname;
			var RutaGlobal = "{{ asset('') }}prueba/boleta";
...
function ModificarApuesta(btn)
{
	...
	switch (Resultado) {
			case "L" :
				$('#resultadoApuesta').append('<option value="L"  selected="true">Local</option>');
				$('#resultadoApuesta').append('<option value="E">Empate</option>');
				$('#resultadoApuesta').append('<option value="V">Visitante</option>');		

				break;
			case "E" :
				$('#resultadoApuesta').append('<option value="L">Local</option>');
				$('#resultadoApuesta').append('<option value="E"  selected="true">Empate</option>');
				$('#resultadoApuesta').append('<option value="V">Visitante</option>');			
			...
			case "V" :
				$('#resultadoApuesta').append('<option value="L">Local</option>');
				$('#resultadoApuesta').append('<option value="E">Empate</option>');
				$('#resultadoApuesta').append('<option value="V"  selected="true">Visitante</option>');				
...
$("#confirmarActualizarApuesta").click(function(){
	...
	var resultado = $('#ResultadoOcul').val();
	//console.log("resultado "+resultado);
	if( resultado == 0 )
	{
		document.getElementById('mensajeActualizarEstadoApuesta').innerHTML="Debe seleccionar un resultado";
	}
	else
	{
